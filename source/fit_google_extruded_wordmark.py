...
def warped(template,p):
 ...
 for q in template:
  ...
  for t in np.linspace(0,1,9):
   mm=m.copy();mm[0,2]-=depth*ZOOM*t;mm[1,2]+=depth*ZOOM*.55*t
   render=cv2.bitwise_or(render,cv2.warpAffine(q,mm,(W,H),flags=cv2.INTER_NEAREST))
  out.append(render)
 return out
...
for name,tpl in [('2013',old),('2015',new)]:
 result=differential_evolution(lambda p:objective(p,tpl),[(0,26),(-8,20),(100,140),(20,50),(90,170),(0,8)],seed=42,popsize=8,maxiter=65,tol=.002,polish=False,workers=1)
 ...
 Image.fromarray(out).save(R/'matching'/f'google_logo_{name}_extruded_fit.png')
out=np.full((H,W,3),230,np.uint8)
...
(R/'matching/google_logo_extruded_fit.json').write_text(json.dumps(results,indent=2))
